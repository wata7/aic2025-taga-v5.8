  ...
  steering_tire_angle_gain_(declare_parameter<float>("steering_tire_angle_gain", 1.0))
{
  ...
  RCLCPP_INFO(this->get_logger(), "SimplePurePursuit initialized with external_target_vel=%.2f", external_target_vel_);
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  for (const auto & param : parameters) {
    if (param.get_name() == "external_target_vel") {
      ...
      RCLCPP_INFO(this->get_logger(), "✅ lookahead_min_distance updated to %.2f", lookahead_min_distance_);
    }
